...
async function startServer() {
  ...
  async function ensureCompilerInstalled() {
    ...
    try {
      ...
      try {
        ...
        console.log("[SETUP] arduino-cli not found. Attempting to install...");
      }
    }
    if (!hasCli) {
      try {
    ...
    if (hasCli) {
      try {
        ...
        await execAsync(`${cliCmd} core update-index --additional-urls ${esp32Url}`).catch(() => {});
        let isEsp32Installed = false;
        try { const { stdout } = await execAsync(`${cliCmd} core list`); if (stdout.includes("esp32:esp32")) isEsp32Installed = true; } catch {}
        if (!isEsp32Installed) {
          console.log("[SETUP] Installing esp32:esp32 core...");
  ...
  const ALL_LIBRARIES = [
    ...
    { id: "httpclient", name: "HTTPClient", version: "2.2.0", author: "Arduino", description: "Make HTTP requests from Arduino.", sentence: "Communication", category: "Communication" },
    { id: "websocket", name: "WebSockets", version: "2.3.6", author: "Links2004", description: "WebSocket client and server.", sentence: "Communication", category: "Communication" },
    ...
    { id: "ssd1306", name: "Adafruit SSD1306", version: "2.5.7", author: "Adafruit", description: "128x64 monochrome OLED display.", sentence: "Display", category: "Display" },
    { id: "bme280", name: "Adafruit BME280", version: "2.2.2", author: "Adafruit", description: "Temperature, humidity, pressure sensor.", sentence: "Sensors", category: "Sensors" },
    { id: "bmp280", name: "Adafruit BMP280", version: "2.6.8", author: "Adafruit", description: "Digital pressure and temperature.", sentence: "Sensors", category: "Sensors" },
    { id: "mpu6050", name: "Adafruit MPU6050", version: "2.0.8", author: "Adafruit", description: "6-axis accelerometer + gyroscope.", sentence: "Sensors", category: "Sensors" },
    ...
    { id: "adxl345", name: "Adafruit ADXL345", version: "1.3.1", author: "Adafruit", description: "3-axis accelerometer.", sentence: "Sensors", category: "Sensors" },
    { id: "onewire", name: "OneWire", version: "2.3.7", author: "Paul Stoffregen", description: "OneWire protocol for Dallas sensors.", sentence: "Sensors", category: "Sensors" },
    { id: "dallastemperature", name: "DallasTemperature", version: "3.9.1", author: "Miles Burton", description: "DS18B20 temperature sensors.", sentence: "Sensors", category: "Sensors" },
    ...
    { id: "stepper", name: "Stepper", version: "1.1.3", author: "Arduino", description: "Control stepper motors.", sentence: "Control", category: "Control" },
    { id: "eeprom", name: "EEPROM", version: "2.0.0", author: "Arduino", description: "Read/write persistent flash data.", sentence: "Storage", category: "Storage" },
    ...
    { id: "spiffs", name: "SPIFFS", version: "2.0.0", author: "Espressif", description: "SPI Flash File System.", sentence: "Storage", category: "Storage" },
    { id: "preferences", name: "Preferences", version: "2.0.0", author: "Espressif", description: "Non-volatile key-value storage.", sentence: "Storage", category: "Storage" },
    { id: "esp32asyncwebserver", name: "ESP32AsyncWebServer", version: "1.3.0", author: "Me-No-Dev", description: "Async web server with WebSocket.", sentence: "Networking", category: "Networking" },
  ...
  async function scanCustomLibraries(): Promise<typeof ALL_LIBRARIES> {
    ...
    try {
      const entries = await fs.readdir(CUSTOM_LIBRARIES_DIR, { withFileTypes: true });
      for (const entry of entries) {
        if (entry.isDirectory()) {
          try {
            const props = Object.fromEntries((await fs.readFile(path.join(CUSTOM_LIBRARIES_DIR, entry.name, "library.properties"), "utf-8")).split("\n").filter(l => l.includes("=")).map(l => { const [k, ...v] = l.split("="); return [k.trim(), v.join("=").trim()]; }));
          ...
          } catch { custom.push({ id: `custom-${entry.name}`, name: entry.name, version: "0.0.0", author: "Custom", description: "Custom imported library", sentence: "Custom", category: "Custom" }); }
        }
      }
    } catch {}
    return custom;
  ...
  app.post("/api/libraries/install", async (req, res) => {
    const { id } = req.body; if (!installedLibraries.includes(id)) { installedLibraries.push(id); await saveState(); } res.json({ success: true, installedLibraries });
  });
  app.post("/api/libraries/uninstall", async (req, res) => {
    const { id } = req.body; installedLibraries = installedLibraries.filter(libId => libId !== id); await saveState(); res.json({ success: true, installedLibraries });
  });
  app.post("/api/libraries/upload-zip", upload.single("library"), async (req: any, res) => {
    if (!req.file) return res.status(400).json({ success: false, error: "No file uploaded" });
    try {
      await fs.mkdir(CUSTOM_LIBRARIES_DIR, { recursive: true });
      const zipPath = req.file.path; const libName = path.parse(req.file.originalname || "library").name;
      const extractDir = path.join(CUSTOM_LIBRARIES_DIR, libName);
      await fs.mkdir(extractDir, { recursive: true });
      await execAsync(`unzip -o "${zipPath}" -d "${extractDir}"`); await fs.unlink(zipPath);
      res.json({ success: true, message: `Library '${libName}' imported successfully` });
    } catch { res.status(500).json({ success: false, error: "Failed to import library" }); }
  });

  app.get("/api/compiler/status", async (_req, res) => {
    let hasArduinoCli = false; let hasEsp32 = false;
    try { let cliCmd = "arduino-cli"; try { await execAsync("./arduino-cli version"); cliCmd = "./arduino-cli"; } catch { await execAsync("arduino-cli version"); } hasArduinoCli = true;
      try { const { stdout } = await execAsync(`${cliCmd} core list`); hasEsp32 = stdout.includes("esp32"); } catch {} } catch {}
    res.json({ compilerReady: hasArduinoCli, esp32CoreInstalled: hasEsp32, message: hasArduinoCli ? (hasEsp32 ? "Ready to compile" : "ESP32 core installing...") : "Compiler not available — ensure the server has arduino-cli installed" });
  });

  // REAL COMPILATION ONLY - NO SIMULATION
  app.post("/api/compile", async (req, res) => {
    const { code, boardId, psramMode, flashSize, usbCdcMode, partitionScheme } = req.body;
    let tempDir: string | null = null;
    try {
      let hasArduinoCli = false; let cliCmd = "arduino-cli";
      try { await execAsync("./arduino-cli version"); hasArduinoCli = true; cliCmd = "./arduino-cli"; } catch { try { await execAsync("arduino-cli version"); hasArduinoCli = true; cliCmd = "arduino-cli"; } catch {} }
      if (!hasArduinoCli) return res.status(503).json({ success: false, logs: "Compilation server unavailable: arduino-cli is not installed on the server. The server must have arduino-cli configured and running to compile code. No simulation is available." });
      const uniqueId = `Sketch_${Date.now()}`; tempDir = path.join("/tmp", uniqueId);
      await fs.mkdir(tempDir, { recursive: true });
      await fs.writeFile(path.join(tempDir, `${uniqueId}.ino`), code || "", "utf-8");
      const outputDir = path.join(tempDir, "build"); await fs.mkdir(outputDir, { recursive: true });
      ...
      let fqbnWithConfig = fqbn; if (buildFlags.length > 0) fqbnWithConfig += `:${buildFlags.join(",")}`;
      const cmd = `${cliCmd} compile --fqbn ${fqbnWithConfig} --output-dir "${outputDir}" "${tempDir}" --additional-urls https://raw.githubusercontent.com/espressif/arduino-esp32/gh-pages/package_esp32_index.json 2>&1`;
      ...
      const flashMatch = fullOutput.match(/(\d+)%.*used.*flash/i); const ramMatch = fullOutput.match(/(\d+)%.*used.*ram/i);
      if (flashMatch) flashPct = `${flashMatch[1]}%`; if (ramMatch) ramPct = `${ramMatch[1]}%`;
  ...
  app.post("/api/flash", async (req, res) => { res.json({ success: true, message: "Flash ready — use Web Serial esptool.js on frontend", boardId: req.body.boardId }); });
