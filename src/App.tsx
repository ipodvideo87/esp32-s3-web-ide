      ...
      <header className="h-12 bg-[#0a0b0e] border-b border-zinc-800/80 flex items-center justify-between px-3 sm:px-4 shrink-0 z-50 select-none">
        <div className="flex items-center gap-2 sm:gap-3 min-w-0">
          ...
          <div className="hidden sm:flex flex-col">
            ...
            <span className="text-[11px] font-black text-white tracking-tight uppercase">Web IDE</span>
          </div>
          <div className="h-5 w-px bg-zinc-800 hidden sm:block shrink-0" />
          <div className="flex items-center min-w-0">
            {isEditingName ? (
              <input autoFocus value={tempName} onChange={e => setTempName(e.target.value)} onBlur={handleRename} onKeyDown={e => e.key === 'Enter' && handleRename()} className="text-xs font-bold text-white bg-[#15171f] border border-[#00979c]/40 rounded px-2 py-0.5 focus:outline-none max-w-[140px] sm:max-w-[200px]" />
            ) : (
              <span onClick={() => { setTempName(projectName); setIsEditingName(true); }} className="text-xs font-bold text-zinc-200 hover:text-[#00979c] transition-colors cursor-pointer truncate max-w-[100px] sm:max-w-[200px]">{projectName}</span>
            )}
          </div>
        </div>
        <div className="flex items-center gap-2 sm:gap-3 shrink-0">
          <div className="hidden lg:flex items-center gap-2 text-[9px] text-zinc-400 font-mono">
            <span className="flex items-center gap-1 text-emerald-400"><CheckCircle size={9} /> Real Compilation</span>
            <span className="text-zinc-700">|</span>
            <span className="flex items-center gap-1 text-[#00979c]"><Zap size={9} /> arduino-cli</span>
          </div>
        </div>
      </header>
